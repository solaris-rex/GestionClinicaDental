...
// Inicializar Stripe (fuera del componente para no recrear en cada render)
const stripePromise = loadStripe(import.meta.env.VITE_STRIPE_PUBLIC_KEY)
console.log('STRIPE KEY:', import.meta.env.VITE_STRIPE_PUBLIC_KEY?.slice(0, 20))
...
// ─── Contador de tiempo restante para reembolso ───────────────────────────────
function ContadorReembolso({ fechaPago }) {
  ...
  return (
    <p className="text-xs text-amber-600 mt-1 font-medium">
      ⏱ Cancelar con reembolso: {tiempoRestante} restantes
    </p>
  )
}

// ─── Formulario de pago Stripe (componente interno) ──────────────────────────
function FormularioPago({ clientSecret, onPagoExitoso, onCancelar, guardando }) {
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Se cobrará <span className="font-bold text-teal-700">S/ 20.00</span> para confirmar tu cita
          </p>
        </div>
        ...
        <form onSubmit={handlePagar} className="space-y-4">
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              disabled={!stripe || procesando || guardando}
...
function ModalCancelacion({ cita, onConfirmar, onCerrar, cancelando }) {
  const dentroDeplazo = cita.fecha_pago
    ? (new Date() - new Date(cita.fecha_pago)) < 24 * 60 * 60 * 1000
    : false
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <h3 className="text-xl font-bold text-gray-800">
            {tieneReembolso ? 'Cancelar con reembolso' : 'Cancelar cita'}
          </h3>
        </div>
        ...
        {tieneReembolso ? (
          ...
          <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl mb-6 text-sm">
            ❌ Han pasado más de 24 horas desde tu pago. Al cancelar <strong>no se te reembolsará</strong> el adelanto de S/ 20.00.
          </div>
        ) : (
...
export default function PanelPaciente() {
  ...
  // Al presionar "Realizar pago": crea el PaymentIntent y muestra el modal de pago
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-payment-intent`,
        {
          ...
          body: JSON.stringify({
            monto: 2000,
            descripcion: `Reserva cita dental — ${form.fecha} ${form.hora}`,
            metadata: {
      ...
      if (!res.ok || data.error) {
        setError('Error al iniciar el pago: ' + (data.error || 'intenta de nuevo'))
        setGuardando(false)
        return
      }

      // Éxito: mostrar modal. NO llamar setGuardando(false) aquí
      setClientSecret(data.client_secret)
      setPaymentIntentId(data.payment_intent_id)
      setMostrarPago(true)
      setGuardando(false) // ← resetear DESPUÉS de abrir el modal
  ...
  // Cuando el pago es exitoso: crear la cita en la base de datos
  async function handlePagoExitoso(intentId) {
    ...
    try {
      ...
      if (citaError) {
        setError('Pago realizado pero error al crear la cita. Contacta a recepción con tu comprobante.')
        setGuardando(false)
        return
      }
      ...
      setError('Error inesperado. Contacta a recepción.')
    } finally {
      setGuardando(false)
    }
  }

  // Cancelar pago sin completar
  ...
  function handleSolicitarCancelacion(cita) {
    setCitaACancelar(cita)
  }

  // Confirmar cancelación (con o sin reembolso)
  async function handleConfirmarCancelacion() {
    if (!citaACancelar) return
    setCancelando(true)

    try {
      // Si la cita fue pagada, intentar reembolso
      if (citaACancelar.pagado && !citaACancelar.reembolsado && citaACancelar.payment_intent_id) {
        ...
        const res = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/refund-payment`,
          {
            ...
            body: JSON.stringify({
              payment_intent_id: citaACancelar.payment_intent_id,
              fecha_pago: citaACancelar.fecha_pago,
            }),
          }
        )
        ...
        // Actualizar cita con resultado del reembolso
        await supabase.from('citas').update({
          estado: 'cancelada',
          reembolsado: refundData.reembolsado === true,
        }).eq('id', citaACancelar.id)
      } else {
        // Cita sin pago: solo cancelar
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        {exitoCita && (
          <div className="bg-green-100 border border-green-300 text-green-700 px-4 py-3 rounded-xl mb-6 flex items-center gap-2">
            ✅ ¡Cita reservada y pago confirmado! Te esperamos. Tienes 24h para cancelar con reembolso.
          </div>
        )}
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex gap-4 items-start">
                        ...
                        <div>
                          ...
                          {cita.reembolsado && (
                            <p className="text-xs text-green-600 mt-1">✅ Reembolso procesado</p>
                          )}
                          {/* Contador de reembolso */}
                          {cita.pagado && !cita.reembolsado &&
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        {/* Botón cancelar (solo si no está cancelada ni completada) */}
                        {cita.estado !== 'cancelada' && cita.estado !== 'completada' && (
                          <button
                            onClick={() => handleSolicitarCancelacion(cita)}
